...
export type LaskentaMachineSnapshot = SnapshotFrom<typeof laskentaMachine>;

export type LaskentaActorRef = ActorRefFrom<typeof laskentaMachine>;
...
const rejectWithError = (message: string) => Promise.reject(new Error(message));

const successToast = (context: LaskentaContext): Toast => {
  const valinnanvaiheSelected = Boolean(
    context.laskentaParams?.valinnanvaiheNimi,
  );
  return {
    key: 'laskenta-success',
    message: valinnanvaiheSelected
      ? 'valinnanhallinta.valmisvalinnanvaihe'
      : 'valinnanhallinta.valmis',
    type: 'success',
    messageParams: valinnanvaiheSelected
      ? { nimi: context.laskentaParams?.valinnanvaiheNimi ?? '' }
      : undefined,
  };
};

export const laskentaMachine = setup({
  types: {
  ...
  actions: {
    // Toteutus annetaan useLaskentaActorRef-hookissa .provide()-kutsulla.
    // Parametrien tyypit määrittävät actionin params-tyypin setup-funktiossa.
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    notify: (_, _toast: Toast) => {},
  },
  actors: {
    ...
    pollLaskenta: fromPromise(({ input }: { input?: StartedLaskentaInfo }) => {
      return input
    ...
    fetchSummary: fromPromise(({ input }: { input?: StartedLaskentaInfo }) => {
      return input
    ...
    stopLaskenta: fromPromise(({ input }: { input?: StartedLaskentaInfo }) => {
      return input
        ? keskeytaLaskenta({
            ...
            [LaskentaState.COMPLETED]: {
              ...
              entry: enqueueActions(({ context, enqueue }) => {
                if (!hasUnfinishedHakukohteet(context)) {
                  enqueue({ type: 'notify', params: successToast(context) });
                }
              }),
            },
          },
